...
{
	const rules = Object.entries(dto).map(([key, value]) =>
	{
		if (value === undefined)
		{
			return [];
		...
		switch (key)
		{
			...
			case 'gender':      return Rules.checkGender();
			case 'orientation': return Rules.checkOrientation();
			case 'biography':   return Rules.checkBiography();
			case 'location':    return [ Rules.checkLatitude(), Rules.checkLongitude() ];
			default:            return [];
		}
	})
	.flat();
