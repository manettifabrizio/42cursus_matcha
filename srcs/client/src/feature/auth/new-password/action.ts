...
export const action: ActionFunction = async ({ request }) => {
	...
	const req = store.dispatch(authApi.endpoints.edit_password.initiate(fields));
	...
	try {
		...
		if (isRTKQFetchBaseQueryError(error)) {
			const registerError = error.data as ApiErrorResponse<NewPasswordError>;

			if ("cause" in registerError.error) {
				return { username: [registerError.error.cause] };
			}

			toast.error(`Error while creating the account: ${registerError.error}`);
